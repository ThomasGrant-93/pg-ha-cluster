...
RUN python3 -m venv /venv && /venv/bin/pip install --no-cache-dir \
    psycopg==3.2.4 \
    patroni[etcd3,psycopg3,aws,barman] && \
...
COPY --chown=postgres:postgres ./ssh-keys/id_rsa_postgres  /var/lib/barman/.ssh/id_rsa
COPY --chown=postgres:postgres ./ssh-keys/id_rsa_postgres.pub  /var/lib/barman/.ssh/id_rsa.pub
COPY --chown=postgres:postgres ./ssh-keys/id_rsa_postgres.pub /var/lib/barman/.ssh/authorized_keys

COPY --chown=postgres:postgres ./ssh-keys/etc /etc
...
RUN touch /var/log/barman/barman.log && \
    chmod 0644 /var/log/barman/barman.log && \
...
RUN usermod -aG root,postgres barman
